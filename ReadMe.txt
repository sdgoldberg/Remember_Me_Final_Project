...
Bug Report
I was unable to make the program write back to the input file before project deadline. 
...
Future Works
The original idea for this project was to make an interactive graph of contacts in order to help users maintain their visual-to-name memory recall of new and old piers. In the future, the application will contain a graph with contacts that point to their mutual friends. The contacts will also all save to a new output file as a local data base. Additionally there will be more formats to type in a contact information instead of only allowing name, phone number, and photo to be in the input file. I would also like to make the project look nicer in the future after learning more about css styling.
